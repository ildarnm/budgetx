...
import { budgets_db, records_db, sections_db } from "@shared/repositories/mocks";

type PartialUpdateBudget = Partial<Budget> & { id: BudgetId };

export interface createBudgetPayload { budget: Budget, sections: Section[], records: Record[] }
...
export class BudgetRepository extends Repository<Budget> {
  ...
  public async create({ budget, sections, records }: createBudgetPayload): Promise<Budget> {
    return delay((resolve) => {
